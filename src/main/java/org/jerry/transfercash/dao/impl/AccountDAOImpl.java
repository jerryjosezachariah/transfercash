...
public class AccountDAOImpl implements AccountDAO {
	...
	private final static String SQL_GET_ACC_BY_USERNAME = "SELECT * FROM Account WHERE UserName = ? ";
	 ...
	 */
	@Override
	public List<Account> getAccountsByUserName(String userName) throws CustomException {
		Connection conn = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
		...
		try {
			conn = H2DAOFactory.getConnection();
			stmt = conn.prepareStatement(SQL_GET_ACC_BY_USERNAME);
			...
			while (rs.next()) {
				Account acc = new Account(rs.getLong("AccountId"), rs.getString("UserName"),
						rs.getBigDecimal("Balance"), rs.getString("CurrencyCode"));
				if (log.isDebugEnabled())
					log.debug("getAccountsByUserName(): Get  Account " + acc);
				accounts.add(acc);
			}
			return accounts;
		} catch (SQLException e) {
