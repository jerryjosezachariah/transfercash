...
import org.jerry.transfercash.dao.impl.AccountDAOImpl;
import org.jerry.transfercash.dao.impl.TransactionDAOImpl;
import org.jerry.transfercash.dao.impl.UserDAOImpl;
import org.jerry.transfercash.utils.Utils;
...
public class H2DAOFactory extends DAOFactory {
	...
	@Override
	public void depopulateTestData() {
		log.info("Depopulating Test User Table and data ..... ");
		Connection conn = null;
		try {
			conn = H2DAOFactory.getConnection();
			RunScript.execute(conn, new FileReader("src/test/resources/teardowndemo.sql"));
		} catch (SQLException e) {
			log.error("depopulateTestData(): Error depopulating user data: ", e);
			throw new RuntimeException(e);
		} catch (FileNotFoundException e) {
			log.error("depopulateTestData(): Error finding test script file ", e);
			throw new RuntimeException(e);
		} finally {
			DbUtils.closeQuietly(conn);
		}
