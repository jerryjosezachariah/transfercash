...
public class TestUserService extends TestService {
    ...
    */
    ...
    public void testGetUser() throws IOException, URISyntaxException {
        ...
        assertTrue(user.getUserName().equals("Harry Potter"));
        assertTrue(user.getEmailAddress().equals("hpotter@gmail.com"));

    }

 @Test
 public void testGetNonExistingUserById() throws IOException, URISyntaxException {
     URI uri = builder.setPath("/users/1000").build();
     HttpGet request = new HttpGet(uri);
     HttpResponse response = client.execute(request);
     int statusCode = response.getStatusLine().getStatusCode();
	...
	@Test
	public void testGetUserByName() throws CustomException, ParseException, IOException, URISyntaxException {
        URI uri = builder.setPath("/users/username/Harry Potter").build();
        HttpGet request = new HttpGet(uri);
        HttpResponse response = client.execute(request);
        int statusCode = response.getStatusLine().getStatusCode();
        assertTrue(statusCode == 200);
	...
	@Test
	public void testGetNonExistingUserByName() throws CustomException, URISyntaxException, ClientProtocolException, IOException {
        URI uri = builder.setPath("/users/username/NonExistingUser").build();
        HttpGet request = new HttpGet(uri);
        HttpResponse response = client.execute(request);
        int statusCode = response.getStatusLine().getStatusCode();
        assertTrue(statusCode == 404);
    ...
    */
    ...
    public void testCreateExistingUser() throws IOException, URISyntaxException {
        URI uri = builder.setPath("/users").build();
        User user = new User("Harry Potter", "hpotter@gmail.com");
        String jsonInString = mapper.writeValueAsString(user);
        StringEntity entity = new StringEntity(jsonInString);
        HttpPost request = new HttpPost(uri);
        request.setHeader("Content-type", "application/json");
    ...
    public void testUpdateUser() throws IOException, URISyntaxException {
        URI uri = builder.setPath("/users/2").build();
        User user = new User("test1", "test1123@gmail.com");
        String jsonInString = mapper.writeValueAsString(user);
        StringEntity entity = new StringEntity(jsonInString);
    ...
    */
    ...
    public void testDeleteUser() throws IOException, URISyntaxException {
        URI uri = builder.setPath("/users/3").build();
   ...
   */
    ...
    public void testDeleteNonExistingUser() throws IOException, URISyntaxException {
        URI uri = builder.setPath("/users/300").build();
        HttpDelete request = new HttpDelete(uri);
        request.setHeader("Content-type", "application/json");
        HttpResponse response = client.execute(request);
        int statusCode = response.getStatusLine().getStatusCode();
